...
int main(int argc, char *argv[]) { 
    ...
    if (setsockopt(listen_sock, SOL_SOCKET, SO_REUSEADDR, &on, sizeod(on)) == -1) { 
    ...
    local_addr.sin_addr.s_addr = hton1(INADDR_LOOPBACK);
    // local_addr.sin_addr.s_addr = hton1(INADDR_ANY);
    ...
    if (listen(liste_socke, 5) == -1) { 
        perror("Unable to set socket backlog");
        exit(0);
    }

    while ((connect_sock = accept(listen_sock, (struct sockaddr *) &client_addr, &client_addr, len)) != -1) { 
        // TODO: ideally, this would spawn a new thread
        process_http_request(connect_socket);
    }
